...
class tl_crm_customer extends Backend
{
    ...
    public function addIcon(
        ...
        $args[0] = sprintf(
            '<div class="list_icon_new" style="background-image:url(\'%ssystem/themes/%s/images/%s.gif\')" data-icon="%s.gif" data-icon-disabled="%s.gif">&nbsp;</div>',
            TL_ASSETS_URL,
            Backend::getTheme(
            ),
            $image,
            $disabled ? $image : rtrim(
                $image,
                '_'
            ),
            rtrim(
     ...
     */
    public function storeDateAdded(
        $dc
    ): void {
        // Front end call
        ...
        $this->Database->prepare(
            'UPDATE tl_crm_customer SET dateAdded=? WHERE id=?'
        )
            ->execute(
                time(
                ),
    ...
    public function toggleVisibility(
        ...
        if (!$this->User->hasAccess(
            ...
            $this->log(
                'Not enough permissions to activate/deactivate member ID "'.$intId.'"',
                __METHOD__,
                TL_ERROR
            );
            $this->redirect(
        ...
        if (is_array(
            $GLOBALS['TL_DCA']['tl_crm_customer']['fields']['disable']['save_callback']
        )) {
            foreach ($GLOBALS['TL_DCA']['tl_crm_customer']['fields']['disable']['save_callback'] as $callback) {
                if (is_array(
                    ...
                    $blnVisible = $this->{$callback[0]}->{$callback[1]}(
                        $blnVisible,
                        ($dc ?: $this)
                    );
                } elseif (is_callable(
        ...
        // Update the database
        $this->Database->prepare(
            "UPDATE tl_crm_customer SET tstamp=$time, disable='".($blnVisible ? '' : 1)."' WHERE id=?"
        )
            ->execute(
                $intId
            );
        ...
        if (!$blnVisible) {
            $this->Database->prepare(
                "DELETE FROM tl_session WHERE name='FE_USER_AUTH' AND pid=?"
            )
                ->execute(
                    $intId
                );
        ...
        if (in_array(
            ...
            $objUser = $this->Database->prepare(
                'SELECT email FROM tl_crm_customer WHERE id=?'
            )
                ->limit(
                    1
                )
            ...
            if ($objUser->numRows) {
                $this->Database->prepare(
                    "UPDATE tl_newsletter_recipients SET tstamp=$time, active=? WHERE email=?"
                )
                    ->execute(
                        ($blnVisible ? 1 : ''),
                        $objUser->email
