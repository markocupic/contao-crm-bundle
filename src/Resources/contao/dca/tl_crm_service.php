...
$GLOBALS['TL_DCA']['tl_crm_service'] = [
    ...
    'fields'      => [
        ...
        'defaultInvoiceText'     => [
            ...
            'inputType' => 'textarea',
            'exclude'   => true,
            'default'   => 'Vielen Dank für Ihren sehr geschätzten Auftrag. Für Rückfragen stehe ich Ihnen gerne zur Verfügung.'.chr(
                    10
                ).chr(
                    10
                ).'Mit besten Grüßen'.chr(
                    10
                ).chr(
                    10
                ).'Marko Cupic',
...
class tl_crm_service extends Backend
{
    ...
    public function generateInvoice(
        ...
        if ('generateInvoice' === Input::get(
            ...
            if (null !== ($objInvoice = CrmServiceModel::findByPk(
                    Input::get(
                        'id'
                    )
                ))) {
                $objInvoiceGenerator = System::getContainer(
                )->get(
                    Generator::class
                );
                $objInvoiceGenerator->generateInvoice(
                    $objInvoice,
                    $type
                );
            }
    ...
    ): void
    {
        ...
        System::getContainer(
            ...
            Message::addInfo(
                'Please read the README.md in vendor/markocupic/contao-crm-bundle and add your Cloudconvert API key in config/config.yml for downloading pdf invoices.'
            );
        }
    }
...
<div class="tl_content_left %s" title="%s">
    ...
    <div class="list-service-row-6">%s: %s %s</div>
        ...
        // Service positions
        $servicePositions = StringUtil::deserialize(
            $arrRow['servicePositions'],
            true
        );
        $quantity = 0;
        $unit = '';
